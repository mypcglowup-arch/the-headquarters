  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className="font-display font-semibold text-sm tracking-widest uppercase hidden md:inline"
          style={{
        ...
        {/* Mobile : compact "QG" mark next to the dots so the logo isn't just abstract */}
        <span
          className="md:hidden font-display font-bold text-[13px] tracking-widest uppercase"
          style={{
            color: darkMode ? '#e2e8f0' : '#0f172a',
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          style={{ ...iconBtn(false), padding: '8px' }}
        >
          <Menu size={20} />
        </button>
      </div>

      {/* ── Slide-in drawer (mobile only, but rendered always so it can */}
      {/*    animate out cleanly when closed). Lives outside the header  */}
      {/*    flow as a fixed overlay.                                      */}
      {isMobile && (
        <MobileDrawer
          open={menuOpen}
          onClose={() => setMenuOpen(false)}
          darkMode={darkMode}        onToggleDark={onToggleDark}
          thinkingMode={thinkingMode} onToggleThinking={onToggleThinking}
          voiceMode={voiceMode}       onToggleVoice={onToggleVoice}
          lang={lang}                onToggleLang={onToggleLang}
          screen={screen}
          onGoHome={onGoHome}
          onGoDashboard={onGoDashboard}     onGoProspects={onGoProspects}
          onGoDecisions={onGoDecisions}     onGoJournal={onGoJournal}
          onGoLibrary={onGoLibrary}         onGoSituations={onGoSituations}
          onGoVictories={onGoVictories}     onGoProfile={onGoProfile}
          onGoWorkflow={onGoWorkflow}       onGoEmail={onGoEmail}
